...
import com.andrei1058.bedwars.libs.sidebar.*;
        ...
        for (String line : lineArray) {
            ...
            if (null != arena) {
                ...
                for (ITeam currentTeam : arena.getTeams()) {
                    ...
                    // Check if this line contains this team's status placeholder
                    if (line.contains("{Team" + currentTeam.getName() + "Status}")) {
                        // Replace team-specific placeholders only for the matching team
                        ...
                        // Replace {team_size} for this team
                        line = line.replace("{team_size}", String.valueOf(currentTeam.getSize()));
                        
                        // Replace {team_you} only if player is member of this team
                        if (isMember) {
                            line = line.replace("{team_you}", getMsg(getPlayer(), Messages.FORMATTING_SCOREBOARD_YOUR_TEAM));
                        } else {
                            line = line.replace("{team_you}", "");
                        }
                    }
                }
