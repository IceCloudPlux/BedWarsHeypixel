...
public class ShopCategory {
    ...
    // 当前正在加载的目标商店索引
    private static ShopIndex currentLoadingShop = null;
    
    /**
     * 设置当前正在加载的商店索引（用于槽位冲突检查）
     */
    public static void setCurrentLoadingShop(ShopIndex shopIndex) {
        currentLoadingShop = shopIndex;
    }
    ...
    public ShopCategory(String path, YamlConfiguration yml) {
        ...
        // 使用当前正在加载的商店索引进行槽位冲突检查
        ShopIndex targetShop = currentLoadingShop != null ? currentLoadingShop : ShopManager.shop;
        for (ShopCategory sc : targetShop.getCategoryList()){
            if (sc.getSlot() == slot){
