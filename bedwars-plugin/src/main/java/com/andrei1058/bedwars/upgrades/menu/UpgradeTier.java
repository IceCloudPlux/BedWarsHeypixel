        ...
        for (String action : UpgradesManager.getConfiguration().getYml().getStringList(parentName + "." + name + ".receive")) {
            ...
            switch (type[0].trim().toLowerCase()) {
                ...
                case "respawn-reduction":
                    ...
                    double reduction;
                    try {
                        reduction = Double.parseDouble(data[0]);
                    } catch (Exception exc) {
                        BedWars.plugin.getLogger().warning("Invalid respawn reduction at upgrades2: " + parentName + "." + name);
                        continue;
                    }
                    ua = new RespawnReductionAction(reduction);
                    break;
                case "command":
