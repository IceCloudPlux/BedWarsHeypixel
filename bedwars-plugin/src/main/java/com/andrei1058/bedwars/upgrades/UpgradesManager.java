...
public class UpgradesManager {
    ...
    //store lower case names for upgrades3
    private static final HashMap<String, UpgradesIndex> menuByName3 = new HashMap<>();

    private static final HashMap<IArena, UpgradesIndex> customMenuForArena = new HashMap<>();

    private static UpgradesConfig upgrades;
    private static UpgradesConfig upgrades3;
    ...
    public static void init() {
        ...
        // 加载 upgrades2 配置（用于 Solo 和 Double）
        ...
        // 加载 upgrades3 配置（用于其他游戏模式）
        upgrades3 = new UpgradesConfig("upgrades3", plugin.getDataFolder().getPath());
        if (upgrades3.getYml().getConfigurationSection("") != null) {
            for (String index : upgrades3.getYml().getConfigurationSection("").getKeys(false)) {
                name = index;
                if (index.startsWith("upgrade-")) {
                    if (getMenuContent(name) == null && !loadUpgradeFromConfig3(name)) {
                        Bukkit.getLogger().log(Level.WARNING, "Could not load upgrade3: " + name);
                    }
                } else if (index.startsWith("separator-")) {
                    if (getMenuContent(name) == null && !loadSeparatorFromConfig3(name)) {
                        Bukkit.getLogger().log(Level.WARNING, "Could not load separator3: " + name);
                    }
                } else if (index.startsWith("category-")) {
                    if (getMenuContent(name) == null && !loadCategoryFromConfig3(name)) {
                        Bukkit.getLogger().log(Level.WARNING, "Could not load category3: " + name);
                    }
                } else if (index.startsWith("base-trap-")) {
                    if (getMenuContent(name) == null && !loadBaseTrapFromConfig3(name)) {
                        Bukkit.getLogger().log(Level.WARNING, "Could not base trap3: " + name);
                    }
                } else if (index.endsWith("-upgrades-settings")) {
                    name = index.replace("-upgrades-settings", "");
                    if (!name.isEmpty()) {
                        if (!loadMenu3(name)) {
                            Bukkit.getLogger().log(Level.WARNING, "Could not load menu3: " + name);
                        }
                    }
                }
            }
    ...
    public static UpgradesIndex getMenuForArena(IArena arena) {
        ...
        // 根据游戏类型选择配置
        // Solo 或 Double 使用 upgrades2，其他使用 upgrades3
        int maxInTeam = arena.getMaxInTeam();
        String groupName = arena.getGroup().toLowerCase();
        
        if (maxInTeam <= 2) {
            // Solo 或 Double 模式，使用 upgrades2 配置
            return menuByName.getOrDefault(groupName, menuByName.get("default"));
        } else {
            // 其他模式，使用 upgrades3 配置
            return menuByName3.getOrDefault(groupName, menuByName3.get("default"));
        }
    }
    ...
    /**
     * Load a menu from upgrades3 config for a group.
     *
     * @param groupName arena group name.
     * @return false if cannot be loaded.
     */
    private static boolean loadMenu3(String groupName) {
        if (!upgrades3.getYml().isSet(groupName + "-upgrades-settings.menu-content")) return false;
        if (menuByName3.containsKey(groupName.toLowerCase())) return false;
        InternalMenu um = new InternalMenu(groupName);
        for (String component : upgrades3.getYml().getStringList(groupName + "-upgrades-settings.menu-content")) {
            String[] data = component.split(",");
            if (data.length <= 1) continue;

            MenuContent mc = getMenuContent(data[0]);
            if (data[0].startsWith("category-")) {
                if (mc == null && loadCategoryFromConfig3(data[0])) {
                    mc = getMenuContent(data[0]);
                }
            } else if (data[0].startsWith("upgrade-")) {
                if (mc == null && loadUpgradeFromConfig3(data[0])) {
                    mc = getMenuContent(data[0]);
                }
            } else if (data[0].startsWith("trap-slot-")) {
                if (mc == null && loadTrapSlot(data[0])) {
                ...
                if (mc == null && loadSeparatorFromConfig3(data[0])) {
                    mc = getMenuContent(data[0]);
                }
            } else if (data[0].startsWith("base-trap-")) {
                if (mc == null && loadBaseTrapFromConfig3(data[0])) {
                    mc = getMenuContent(data[0]);
                }
            }
            if (mc == null) continue;
        ...
        menuByName3.put(groupName.toLowerCase(), um);
        BedWars.debug("Registering upgrade menu from upgrades3: " + groupName);
        return true;
    }

    /**
     * Load a category from upgrades3 config.
     */
    private static boolean loadCategoryFromConfig3(String name) {
        if (name == null) return false;
        if (!name.startsWith("category-")) return false;
        if (upgrades3.getYml().get(name) == null) return false;
        if (getMenuContent(name) != null) return false;
        MenuCategory uc = new MenuCategory(name, createDisplayItem3(name));
        for (String component : upgrades3.getYml().getStringList(name + ".category-content")) {
            String[] data = component.split(",");
            if (data.length <= 1) continue;
        ...
        menuContentByName.put(name.toLowerCase(), uc);
        BedWars.debug("Registering upgrade from upgrades3: " + name);
        return true;
    }

    /**
     * Load an upgrade from upgrades3 config.
     */
    private static boolean loadUpgradeFromConfig3(String name) {
        if (name == null) return false;
        if (!name.startsWith("upgrade-")) return false;
        if (upgrades3.getYml().get(name) == null) return false;
        if (upgrades3.getYml().get(name + ".tier-1") == null) return false;
        if (getMenuContent(name) != null) return false;
        MenuUpgrade mu = new MenuUpgrade(name);

        for (String s : upgrades3.getYml().getConfigurationSection(name).getKeys(false)) {
            if (!s.startsWith("tier-")) continue;
            if (upgrades3.getYml().get(name + "." + s + ".receive") == null) {
                BedWars.debug("Could not load Upgrade " + name + " tier: " + s + ". Receive not set.");
                continue;
            }
            if (upgrades3.getYml().get(name + "." + s + ".display-item") == null) {
                BedWars.debug("Could not load Upgrade " + name + " tier: " + s + ". Display item not set.");
                continue;
            }
            if (upgrades3.getYml().get(name + "." + s + ".cost") == null) {
                BedWars.debug("Could not load Upgrade " + name + " tier: " + s + ". Cost not set.");
                continue;
            }
            if (upgrades3.getYml().get(name + "." + s + ".currency") == null) {
                BedWars.debug("Could not load Upgrade " + name + " tier: " + s + ". Currency not set.");
                continue;
            }
            UpgradeTier ut = new UpgradeTier(name, s, createDisplayItem3(name + "." + s), upgrades3.getYml().getInt(name + "." + s + ".cost"), getCurrency(upgrades3.getYml().getString(name + "." + s + ".currency")));
            if (!mu.addTier(ut)) {
                Bukkit.getLogger().log(Level.WARNING, "Could not load tier: " + s + " at upgrade: " + name);
            }
        }
    ...
    /**
     * Load a separator from upgrades3 config.
     */
    private static boolean loadSeparatorFromConfig3(String name) {
        if (name == null) return false;
        if (!name.startsWith("separator-")) return false;
        if (upgrades3.getYml().get(name) == null) return false;
        if (getMenuContent(name) != null) return false;
        MenuSeparator ms = new MenuSeparator(name, createDisplayItem3(name));
        menuContentByName.put(name.toLowerCase(), ms);
        BedWars.debug("Registering separator from upgrades3: " + name);
        return true;
    }

    /**
     * Load a base trap from upgrades3 config.
     */
    private static boolean loadBaseTrapFromConfig3(String name) {
        if (name == null) return false;
        if (!name.startsWith("base-trap-")) return false;
        if (upgrades3.getYml().get(name) == null) return false;
        if (upgrades3.getYml().get(name + ".receive") == null) {
            BedWars.debug("Could not load BaseTrap. Receive not set.");
            return false;
        }
        if (upgrades3.getYml().get(name + ".display-item") == null) {
            BedWars.debug("Could not load BaseTrap. Display item not set.");
            return false;
        }

        MenuBaseTrap bt = new MenuBaseTrap(name, createDisplayItem3(name), upgrades3.getYml().getInt(name + ".cost"), getCurrency(upgrades3.getYml().getString(name + ".currency")));
        BedWars.debug("Registering base trap from upgrades3: " + name);
        menuContentByName.put(name.toLowerCase(), bt);
        return true;
    }

    /**
     * Create a display item from upgrades3 config.
     */
    private static ItemStack createDisplayItem3(String path) {
        Material m;
        try {
            m = Material.valueOf(upgrades3.getYml().getString(path + ".display-item.material"));
        } catch (Exception e) {
            m = Material.BEDROCK;
        }
        ItemStack i = new ItemStack(m, Integer.parseInt(upgrades3.getYml().getString(path + ".display-item.amount")), (short) upgrades3.getYml().getInt(path + ".display-item.data"));
        if (upgrades3.getYml().getBoolean(path + ".display-item.enchanted")) {
            i.addUnsafeEnchantment(Enchantment.DURABILITY, 1);
            ItemMeta im = i.getItemMeta();
            if (im != null) {
                im.addItemFlags(ItemFlag.HIDE_ENCHANTS);
