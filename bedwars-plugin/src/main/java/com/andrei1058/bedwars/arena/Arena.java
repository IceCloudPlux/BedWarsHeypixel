...
public class Arena implements IArena {
    ...
    public Arena(String name, Player p) {
        ...
        // 加载游戏模式
        gameType = GameType.fromIdentifier(yml.getString("gameType"));
                        ...
                        for (Player receiver : receivers) {
                            ...
                            if (winner.getMembers().contains(receiver) || winner.wasMember(receiver.getUniqueId())) {
                                ...
                                if (startTime != null) {
                                    ...
                                    gameTime = formatGameTime((int) durationSeconds);
                                }

                                // Send victory title and subtitle with game time
                                String title = getMsg(receiver, Messages.GAME_END_VICTORY_PLAYER_TITLE);
                                String subtitle = winner.getDisplayName(playerLang) + " 获得胜利, 用时 " + gameTime;
                                nms.sendTitle(receiver, title, subtitle, 0, 70, 20);
                            } else {
    ...
    public void updateNextEvent() {
        ...
        if (nextEvent == NextEvent.EMERALD_GENERATOR_TIER_II && upgradeEmeraldsCount == 0) {
            ...
            } else if (upgradeDiamondsCount < next && diamondTier == 3) {
                setNextEvent(NextEvent.DIAMOND_GENERATOR_TIER_IV);
            } else {
            ...
            if (upgradeEmeraldsCount < next && emeraldTier == 1) {
                ...
                setNextEvent(NextEvent.EMERALD_GENERATOR_TIER_III);
            } else if (upgradeEmeraldsCount < next && emeraldTier == 3) {
                setNextEvent(NextEvent.EMERALD_GENERATOR_TIER_IV);
            } else {
                setNextEvent(NextEvent.DIAMOND_GENERATOR_TIER_III);
            }
            ...
            sendEmeraldsUpgradeMessages();
            // 检查是否有下一个升级
            String tier4Path = getGroup() + "." + ConfigPath.GENERATOR_EMERALD_TIER_IV_START;
            int nextTier4Start = getGeneratorsCfg().getYml().get(tier4Path) == null ?
                    -1 : getGeneratorsCfg().getInt(tier4Path);
            ...
            if (nextTier4Start > 0) {
                if (diamondTier == 1 && upgradeDiamondsCount > 0) {
                    ...
                    setNextEvent(NextEvent.DIAMOND_GENERATOR_TIER_III);
                } else if (diamondTier == 3 && upgradeDiamondsCount > 0) {
                    setNextEvent(NextEvent.DIAMOND_GENERATOR_TIER_IV);
                } else {
                    setNextEvent(NextEvent.EMERALD_GENERATOR_TIER_IV);
                }
            ...
            sendDiamondsUpgradeMessages();
            // 检查是否有下一个升级
            String tier4Path = getGroup() + "." + ConfigPath.GENERATOR_DIAMOND_TIER_IV_START;
            int nextTier4Start = getGeneratorsCfg().getYml().get(tier4Path) == null ?
                    -1 : getGeneratorsCfg().getInt(tier4Path);
            ...
            if (nextTier4Start > 0) {
                if (emeraldTier == 1 && upgradeEmeraldsCount > 0) {
                    ...
                    setNextEvent(NextEvent.EMERALD_GENERATOR_TIER_III);
                } else if (emeraldTier == 3 && upgradeEmeraldsCount > 0) {
                    setNextEvent(NextEvent.EMERALD_GENERATOR_TIER_IV);
                } else {
                    setNextEvent(NextEvent.DIAMOND_GENERATOR_TIER_IV);
                }
        ...
        } else if (nextEvent == NextEvent.EMERALD_GENERATOR_TIER_IV && upgradeEmeraldsCount == 0) {
            emeraldTier = 4;
            sendEmeraldsUpgradeMessages();
            // 检查是否有下一个升级
            String tier5Path = getGroup() + "." + ConfigPath.GENERATOR_EMERALD_TIER_V_START;
            int nextTier5Start = getGeneratorsCfg().getYml().get(tier5Path) == null ?
                    -1 : getGeneratorsCfg().getInt(tier5Path);
            ...
            if (nextTier5Start > 0) {
                if (diamondTier == 2 && upgradeDiamondsCount > 0) {
                    ...
                    setNextEvent(NextEvent.DIAMOND_GENERATOR_TIER_IV);
                } else if (diamondTier == 4 && upgradeDiamondsCount > 0) {
                    setNextEvent(NextEvent.DIAMOND_GENERATOR_TIER_V);
                } else {
                    setNextEvent(NextEvent.EMERALD_GENERATOR_TIER_V);
                }
            } else {
                if (diamondTier == 2 && upgradeDiamondsCount > 0) {
                    setNextEvent(NextEvent.DIAMOND_GENERATOR_TIER_III);
        ...
        } else if (nextEvent == NextEvent.DIAMOND_GENERATOR_TIER_IV && upgradeDiamondsCount == 0) {
            diamondTier = 4;
            sendDiamondsUpgradeMessages();
            // 检查是否有下一个升级
            String tier5Path = getGroup() + "." + ConfigPath.GENERATOR_DIAMOND_TIER_V_START;
            int nextTier5Start = getGeneratorsCfg().getYml().get(tier5Path) == null ?
                    -1 : getGeneratorsCfg().getInt(tier5Path);
            ...
            if (nextTier5Start > 0) {
                if (emeraldTier == 2 && upgradeEmeraldsCount > 0) {
                    ...
                    setNextEvent(NextEvent.EMERALD_GENERATOR_TIER_IV);
                } else if (emeraldTier == 4 && upgradeEmeraldsCount > 0) {
                    setNextEvent(NextEvent.EMERALD_GENERATOR_TIER_V);
                } else {
                    setNextEvent(NextEvent.DIAMOND_GENERATOR_TIER_V);
                }
        ...
        } else if (nextEvent == NextEvent.EMERALD_GENERATOR_TIER_V && upgradeEmeraldsCount == 0) {
            emeraldTier = 5;
            sendEmeraldsUpgradeMessages();
            if (diamondTier == 3 && upgradeDiamondsCount > 0) {
                setNextEvent(NextEvent.DIAMOND_GENERATOR_TIER_IV);
            } else if (diamondTier == 4 && upgradeDiamondsCount > 0) {
                setNextEvent(NextEvent.DIAMOND_GENERATOR_TIER_V);
            } else {
        ...
        } else if (nextEvent == NextEvent.DIAMOND_GENERATOR_TIER_V && upgradeDiamondsCount == 0) {
            diamondTier = 5;
            sendDiamondsUpgradeMessages();
            if (emeraldTier == 3 && upgradeEmeraldsCount > 0) {
                setNextEvent(NextEvent.EMERALD_GENERATOR_TIER_IV);
            } else if (emeraldTier == 4 && upgradeEmeraldsCount > 0) {
                setNextEvent(NextEvent.EMERALD_GENERATOR_TIER_V);
            } else {
...
//        }
    ...
    public boolean startReSpawnSession(Player player, int seconds) {
        if (respawnSessions.get(player) == null) {
            ...
            if (seconds > 1) {
                ...
                // Convert seconds to half-seconds (multiply by 2)
                respawnSessions.put(player, seconds * 2);
    ...
    public boolean addGameTime(int seconds) {
        ...
        if (playingTask instanceof GamePlayingTask) {
            ((GamePlayingTask) playingTask).addTime(seconds);
            return true;
        }
        return false;
    }
    ...
    /**
     * Format game time in seconds to human-readable format
