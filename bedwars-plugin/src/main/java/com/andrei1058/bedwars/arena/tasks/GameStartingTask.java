    ...
    private void spawnPlayers() {
        for (ITeam bwt : getArena().getTeams()) {
            for (Player p : new ArrayList<>(bwt.getMembers())) {
                ...
                // 发送游戏开始标题和副标题
                String title = getMsg(p, Messages.ARENA_STATUS_START_PLAYER_TITLE);
                String subtitle = getMsg(p, Messages.ARENA_STATUS_START_PLAYER_SUBTITLE);
                nms.sendTitle(p, title, subtitle, 0, 30, 10);
