...
public class OreGenerator implements IGenerator {
    ...
    public void upgrade() {
        switch (type) {
            case DIAMOND:
                ...
                } else if (upgradeStage == 4) {
                    delay = getGeneratorsCfg().getInt(getGeneratorsCfg().getYml().get(arena.getGroup() + "." + ConfigPath.GENERATOR_DIAMOND_TIER_IV_DELAY) == null ?
                            "Default." + ConfigPath.GENERATOR_DIAMOND_TIER_IV_DELAY : arena.getGroup() + "." + ConfigPath.GENERATOR_DIAMOND_TIER_IV_DELAY);
                    amount = getGeneratorsCfg().getInt(getGeneratorsCfg().getYml().get(arena.getGroup() + "." + ConfigPath.GENERATOR_DIAMOND_TIER_IV_AMOUNT) == null ?
                            "Default." + ConfigPath.GENERATOR_DIAMOND_TIER_IV_AMOUNT : arena.getGroup() + "." + ConfigPath.GENERATOR_DIAMOND_TIER_IV_AMOUNT);
                    spawnLimit = getGeneratorsCfg().getInt(getGeneratorsCfg().getYml().get(arena.getGroup() + "." + ConfigPath.GENERATOR_DIAMOND_TIER_IV_SPAWN_LIMIT) == null ?
                            "Default." + ConfigPath.GENERATOR_DIAMOND_TIER_IV_SPAWN_LIMIT : arena.getGroup() + "." + ConfigPath.GENERATOR_DIAMOND_TIER_IV_SPAWN_LIMIT);
                } else if (upgradeStage == 5) {
                    delay = getGeneratorsCfg().getInt(getGeneratorsCfg().getYml().get(arena.getGroup() + "." + ConfigPath.GENERATOR_DIAMOND_TIER_V_DELAY) == null ?
                            "Default." + ConfigPath.GENERATOR_DIAMOND_TIER_V_DELAY : arena.getGroup() + "." + ConfigPath.GENERATOR_DIAMOND_TIER_V_DELAY);
                    amount = getGeneratorsCfg().getInt(getGeneratorsCfg().getYml().get(arena.getGroup() + "." + ConfigPath.GENERATOR_DIAMOND_TIER_V_AMOUNT) == null ?
                            "Default." + ConfigPath.GENERATOR_DIAMOND_TIER_V_AMOUNT : arena.getGroup() + "." + ConfigPath.GENERATOR_DIAMOND_TIER_V_AMOUNT);
                    spawnLimit = getGeneratorsCfg().getInt(getGeneratorsCfg().getYml().get(arena.getGroup() + "." + ConfigPath.GENERATOR_DIAMOND_TIER_V_SPAWN_LIMIT) == null ?
                            "Default." + ConfigPath.GENERATOR_DIAMOND_TIER_V_SPAWN_LIMIT : arena.getGroup() + "." + ConfigPath.GENERATOR_DIAMOND_TIER_V_SPAWN_LIMIT);
                }
                ore = new ItemStack(Material.DIAMOND);
                for (IGenHolo e : armorStands.values()) {
                    String tierMsg = getTierMessage(upgradeStage);
                    e.setTierName(Language.getLang(e.getIso()).m(Messages.GENERATOR_HOLOGRAM_TIER).replace("{tier}", Language.getLang(e.getIso()).m(tierMsg)));
                }
                break;
            case EMERALD:
                upgradeStage++;
                if (upgradeStage == 2) {
                    ...
                    delay = getGeneratorsCfg().getInt(getGeneratorsCfg().getYml().get(arena.getGroup() + "." + ConfigPath.GENERATOR_EMERALD_TIER_IV_DELAY) == null ?
                            "Default." + ConfigPath.GENERATOR_EMERALD_TIER_IV_DELAY : arena.getGroup() + "." + ConfigPath.GENERATOR_EMERALD_TIER_IV_DELAY);
                    amount = getGeneratorsCfg().getInt(getGeneratorsCfg().getYml().get(arena.getGroup() + "." + ConfigPath.GENERATOR_EMERALD_TIER_IV_AMOUNT) == null ?
                            "Default." + ConfigPath.GENERATOR_EMERALD_TIER_IV_AMOUNT : arena.getGroup() + "." + ConfigPath.GENERATOR_EMERALD_TIER_IV_AMOUNT);
                    spawnLimit = getGeneratorsCfg().getInt(getGeneratorsCfg().getYml().get(arena.getGroup() + "." + ConfigPath.GENERATOR_EMERALD_TIER_IV_SPAWN_LIMIT) == null ?
                            "Default." + ConfigPath.GENERATOR_EMERALD_TIER_IV_SPAWN_LIMIT : arena.getGroup() + "." + ConfigPath.GENERATOR_EMERALD_TIER_IV_SPAWN_LIMIT);
                } else if (upgradeStage == 5) {
                    delay = getGeneratorsCfg().getInt(getGeneratorsCfg().getYml().get(arena.getGroup() + "." + ConfigPath.GENERATOR_EMERALD_TIER_V_DELAY) == null ?
                            "Default." + ConfigPath.GENERATOR_EMERALD_TIER_V_DELAY : arena.getGroup() + "." + ConfigPath.GENERATOR_EMERALD_TIER_V_DELAY);
                    amount = getGeneratorsCfg().getInt(getGeneratorsCfg().getYml().get(arena.getGroup() + "." + ConfigPath.GENERATOR_EMERALD_TIER_V_AMOUNT) == null ?
                            "Default." + ConfigPath.GENERATOR_EMERALD_TIER_V_AMOUNT : arena.getGroup() + "." + ConfigPath.GENERATOR_EMERALD_TIER_V_AMOUNT);
                    spawnLimit = getGeneratorsCfg().getInt(getGeneratorsCfg().getYml().get(arena.getGroup() + "." + ConfigPath.GENERATOR_EMERALD_TIER_V_SPAWN_LIMIT) == null ?
                            "Default." + ConfigPath.GENERATOR_EMERALD_TIER_V_SPAWN_LIMIT : arena.getGroup() + "." + ConfigPath.GENERATOR_EMERALD_TIER_V_SPAWN_LIMIT);
                }
                ...
                for (IGenHolo e : armorStands.values()) {
                    ...
                    e.setTierName(Language.getLang(e.getIso()).m(Messages.GENERATOR_HOLOGRAM_TIER).replace("{tier}",
                            Language.getLang(e.getIso()).m(tierMsg)));
                }
                break;
        }
    ...
    private String getTierMessage(int tier) {
        switch (tier) {
            case 2:
                return Messages.FORMATTING_GENERATOR_TIER2;
            case 3:
                return Messages.FORMATTING_GENERATOR_TIER3;
            case 4:
                return Messages.FORMATTING_GENERATOR_TIER4;
            case 5:
                return Messages.FORMATTING_GENERATOR_TIER5;
            default:
                return Messages.FORMATTING_GENERATOR_TIER1;
        }
    }
    ...
    public void spawn() {
        ...
        if (lastSpawn == 0) {
            ...
            if (plugin.getConfig().getBoolean(ConfigPath.GENERAL_CONFIGURATION_ENABLE_GEN_SPLIT)) {
                ...
                // 经验模式 - 给附近玩家经验值（资源仍然生成，但拾取后转换为经验）
                if (arena.getGameType() == GameType.XP) {
                    // 先生成物品
                    dropItem(location);
                    // 给站在资源池上的玩家直接增加经验
     ...
     * @param loc 生成位置
     */
    private void giveXpToNearbyPlayers(Location loc) {
    ...
    private int getXpAmount() {
        XpModeConfig xpConfig = XpModeConfig.getInstance();
        if (xpConfig == null) return 0;
        
        switch (type) {
            case IRON:
                return xpConfig.getIronXp() * amount;
            case GOLD:
                return xpConfig.getGoldXp() * amount;
            case EMERALD:
                return xpConfig.getEmeraldXp() * amount;
            case DIAMOND:
                return xpConfig.getDiamondXp() * amount;
            default:
                return 0;
        }
    }
